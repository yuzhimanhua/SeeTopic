...
const int vocab_hash_size = 3000000;  // Maximum 3 * 0.7 = 2.1M words in the vocabulary
const int corpus_max_size = 400000;   // Maximum 400K documents in the corpus
...
void WriteResult() {
  ...
  for (a = 0; a < num_topic; a++) {
    ...
    fprintf(fo, "%ld:%s", a, topics[a].category_name);
    cur_sz = 0;
    for (b = 0; b < vocab_size; b++) {
      flag = 0;
      for (c = 0; c < topics[a].init_size; c++) {
      ...
      if (cur_sz >= num_per_topic-1) break;
    }
    fprintf(fo, "\n");
  }
  fclose(fo);
